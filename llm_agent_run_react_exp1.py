...
from agents.react_llm_agent import ReActAgentNoFSM
...
async def react() -> str:
    ...
    while True:
        if to_react:  ## react with fixed time interval or in case of urgency
            ...
            llm_input = rule_agent.get_react_llm_input()
        ...
        for i, k in enumerate(inp.data.keys()):
            if i == llm_idx:
                if not mid_action:
                    ...
                    if mid_action:
                        logger.warning(f"React Agent: {mid_action}")
                    message_dict = {}
...
if __name__ == "__main__":
    ...
    f = open("logs/llm_agent_react.log", "w")
    logger.add(f, level="TRACE")
    f = open("logs/llm_agent_react_less.log", "w")
    ...
    dir_path = f"results/exp1_2/{env_conf.mode}/react"
    ...
    loop.run_until_complete(asyncio.gather(run_game(), listen_action(), react()))
