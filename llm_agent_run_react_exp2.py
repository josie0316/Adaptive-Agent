        ...
        for i, k in enumerate(inp.data.keys()):
            if i == llm_idx:
                if not mid_action:
                    ...
                    logger.info(f"React Agent: {mid_action}")
                    message_dict = {}
...
if __name__ == "__main__":
    ...
    if FSM and args.no_model:
        ...
        file_path = f"{dir_path}/react/{MODEL}-{args.seed}.json"
    ...
    if args.no_model:
        ...
        loop.run_until_complete(asyncio.gather(run_game(), get_biased_agent_action(), react()))
