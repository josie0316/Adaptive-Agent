...
class SARDConsistencyChecker(Wrapper):  # sub-class MultiagentEnv
    ...
    def step(self, decision):
        assert getattr(self.last_data, "obs", None) is not None, f"data must contain obs field. Found {self.last_data}"
        assert (
            decision.keys() == self.last_data.keys()
...
class TurnBasedConsistencyChecker(SARDConsistencyChecker):
    def step(self, decision):
        ...
        for k, v in self.last_data.items():
            assert (
                ("obs" in v) and ("done" in v) and ("reward" in v)
...
class RescaleActionsWrapper(Wrapper):
    def __init__(self, env, min_action, max_action, *args, **kwargs):
        ...
        super().__init__(env)
        self.min_action = {
            p: np.zeros(env.action_spaces[p].shape, dtype=env.action_spaces[p].dtype) + min_action for p in env.players
        }
        self.max_action = {
            p: np.zeros(env.action_spaces[p].shape, dtype=env.action_spaces[p].dtype) + max_action for p in env.players
        }
    ...
    def step(self, decision):
        ...
        for p in out:
            ...
            action = low + (high - low) * ((action - self.min_action[p]) / (self.max_action[p] - self.min_action[p]))
...
class NormalizedWrapper(Wrapper):
    def __init__(
        ...
        **kwargs,
        ...
        self.env = env
        self.ob_rms = (
            {p: RunningMeanStd(shape=self.get_observation_space().obs.shape) for p in env.players} if norm_obs else None
        )
        self.state_rms = None
        if norm_state:
    ...
    def get_stats(self):
        out = {"ob_rms": deepcopy(self.ob_rms), "ret_rms": deepcopy(self.ret_rms)}
    ...
    def _rewfilt(self, rew, p):
        if self.state_rms and (p in self.state_rms):
            ...
            rew = np.clip(
                rew / np.sqrt(self.ret_rms[p].var + self.epsilon),
                -self.cliprew,
                self.cliprew,
            ).astype(np.float32)
        return rew
    ...
    def _obfilt(self, obs, p):
        if self.state_rms and (p in self.state_rms):
            ...
            obs = np.clip(
                (obs - self.ob_rms[p].mean) / np.sqrt(self.ob_rms[p].var + self.epsilon),
                -self.clipob,
                self.clipob,
            ).astype(np.float32)
        return obs
    ...
    def _statefilt(self, state, p):
        if self.state_rms and (p in self.state_rms):
            ...
            state = np.clip(
                (state - self.state_rms[p].mean) / np.sqrt(self.state_rms[p].var + self.epsilon),
                -self.clipob,
                self.clipob,
            ).astype(np.float32)
        return state
