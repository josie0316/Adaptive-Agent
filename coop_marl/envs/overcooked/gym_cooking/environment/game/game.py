...
class Game:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_RETURN:
                ...
                pygame.image.save(
                    ...
                    f"{self.save_dir}/{image_name}",
                )
                print(f"Saved image {image_name} to {self.save_dir}")
                return
            ...
            if event.key in KeyToTuple_human1 and self.num_humans > 0:
                ...
                self.yielding_action_dict = {
                    agent: self.env.unwrapped.world_agent_mapping[agent].action for agent in self.env.agents
                }
    ...
    def on_execute_yielding(self):
        ...
        while self._running:
            ...
            if self.step_done:
                self.step_done = False
                yield self.store["observation"][-1], self.store["done"][-1], self.store["info"][-1], self.store[
                    "rewards"
                ][-1], self.yielding_action_dict
        self.on_cleanup()
