...
def array(fig):
    ...
    return np.frombuffer(renderer.buffer_rgba(), np.uint8).reshape((h, w, 4))[:, :, :3].copy()


def timedelta_xaxis(f):
    f.xaxis.ticker = bom.tickers.DatetimeTicker()
    f.xaxis.formatter = bom.FuncTickFormatter(
        code="""
...
def suffix_yaxis(f):
    f.yaxis.formatter = bom.FuncTickFormatter(
        code="""
        var min_diff = Infinity;
        for (var i = 0; i < ticks.length-1; i++) {
            min_diff = Math.min(min_diff, ticks[i+1]-ticks[i]);
...
def default_tools(f):
    ...
    f.js_on_event(boe.DoubleTap, bom.callbacks.CustomJS(args=dict(p=f), code="p.reset.emit()"))
...
def _timedataframe(source, x, ys):
    ...
    f.legend.border_line_alpha = 0.0
