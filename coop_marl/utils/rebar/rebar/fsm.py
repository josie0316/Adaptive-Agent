...
class FSM:
    ...
    def solve(self, eps=1e-3, gamma=0.99):
        ...
        while True:
            ...
            if change.pow(2).mean().pow(0.5) < eps:
                break
...
class State:
    ...
    def to(self, state, action=0, reward=0.0, weight=1.0):
        action = int(action)
        self._builder._trans.append(
            dotdict.dotdict(prev=self._name, action=action, next=state, reward=reward, weight=weight)
        )
        return self
...
class Builder:
    ...
    def state(self, name, obs, start=0.0):
    ...
    def build(self):
        states = {x.state for x in self._obs} | {x.prev for x in self._trans} | {x.next for x in self._trans}
        ...
        start = torch.full((n_states,), 0.0)
        for x in self._obs:
            obs[indices[x.state]] = torch.as_tensor(x.obs)
            start[indices[x.state]] = x.start

        trans = torch.full((n_states, n_actions, n_states), 0.0)
        reward = torch.full((n_states, n_actions), 0.0)
...
def ObliviousConstantReward():
    return Builder().state("start", obs=(), start=1.0).to("end", reward=1.0).build()
...
def ObliviousCyclicReward():
    return (
        Builder()
        .state("start", obs=0.0, start=1.0)
        .to("middle", reward=1)
        .state("middle", obs=1.0)
        .to("end", reward=0)
        .build()
    )
...
def ObliviousChain(n=2, r=1):
    ...
    b = Builder()
    b.state(0, obs=0.0, start=1.0).to(1, 0)
    for i in range(1, n):
        reward = i == n - 1
...
def ObliviousCoin():
    return (
        Builder()
        .state("heads", obs=+1.0, start=1.0)
        .to("end", 0, reward=+1)
        .state("tails", obs=-1.0, start=1.0)
        .to("end", 0, reward=-1)
        .build()
    )
...
def ObliviousDelayedCoin():
    return (
        Builder()
        .state("heads-1", obs=+0.5, start=1.0)
        .to("heads-2")
        .state("heads-2", obs=+1.0)
        .to("end", reward=+1)
        .state("tails-1", obs=-0.5, start=1.0)
        .to("tails-2")
        .state("tails-2", obs=-1.0)
        .to("end", reward=-1)
        .build()
    )
...
def DelayedMatchCoin():
    return (
        Builder()
        .state("heads-1", obs=+1.0, start=1.0)
        .to("heads-2", 0)
        .to("heads-2", 1)
        .state("heads-2", obs=0.0)
        .to("end", 0, reward=+1)
        .to("end", 1, reward=-1)
        .state("tails-1", obs=0.0, start=1.0)
        .to("tails-2", 0)
        .to("tails-2", 1)
        .state("tails-2", obs=-1.0)
        .to("end", 0, reward=-1)
...
def RandomChain(n=2, seed=0):
    ...
    (b.state(0, obs=0.0, start=1.0).to(0, action=actions[0]).to(1, action=actions[1]))
    for i in range(1, n):
        reward = i == n - 1
        actions = random.permutation([0, 1])
        (b.state(+i, obs=+i / n).to(0, action=actions[0]).to(i + 1, action=actions[1], reward=+reward))
    return b.build()
